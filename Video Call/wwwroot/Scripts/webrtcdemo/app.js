...
WebRtcDemo.App = (function (viewModel, connectionManager) {
    var _mediaStream,
        ...
        _connect = function (username, onSuccess, onFailure) {
            ...
            var hub = new signalR.HubConnectionBuilder().withUrl("/WebRtcHub").build();
            hub.start().then(function () {
                // Tell the hub what our username is
                console.log('connected to SignalR hub... connection id: ' + _hub.connection.id);
                _hub.invoke("join", username).catch(err => console.error(err));
        ...
        _startSession = function (username) {
            ...
            getUserMedia(
                ...
                function (stream) { // succcess callback gives us a media stream
                    ...
                    _connect(username, function (_hub) {                        
                        ...
                        connectionManager.initialize(_hub, _callbacks.onReadyForStream, _callbacks.onStreamAdded, _callbacks.onStreamRemoved);
        ...
        _attachUiHandlers = function() {
            ...
            $('.user').live('click', function () {
                ...
                if (targetConnectionId != viewModel.MyConnectionId()) {
                    // Initiate a call
                    _hub.invoke("CallUser", targetConnectionId).catch(err => console.error(err));
            ...
            $('.hangup').click(function () {
                ...
                if (viewModel.Mode() != 'idle') {
                    ...
                    hub.invoke("hangUp").catch(err => console.error(err));
    ...
    hub.on("callAccepted", function (acceptingUser) {
            ...
            hub.on("callDeclined", function (decliningConnectionId, reason) {
            ...
            hub.on("updateUserList", function (Users) {
